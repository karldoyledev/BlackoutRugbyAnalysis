...
22. **Route map + navigation (D2)**: routes `/` (Home, `@page "/"`), `/Fixtures/{id:int}` (Match Analysis), `/Players/{id:int}` (Player History), `/Squad` (ex-MemberStatsComparison), `/Playground` (ex-ApiTester), `/Account/{SignUp,Login,LinkClub,Settings}`; no Logout route — sign-out is a POST handler on Settings; default-deny via `AuthorizeFolder("/")`, anonymous only on Login + SignUp; link-required = Fixtures, Players, Squad (enforced in code, not route config) — Unlinked users → Home prompt card, Linked users on LinkClub → Settings; Playground auth-only, not link-required; no id-ownership checks in v1 (foreign ids degrade gracefully: empty trends / raw API data); landing: anonymous `/` → Login, login honors local `returnUrl` else Home, sign-up → LinkClub, link save → Home; stale credentials → decision-2 panel (runtime, not a route); gate-driven conditional nav (anonymous: brand + Log in/Sign up; unlinked: Home · API Playground · Settings; linked: Home · Squad · Player History · API Playground · Settings), brand unchanged, sign-out POST on Settings only; legacy: Index + Privacy deleted, ApiTester + MemberStatsComparison moved, no redirect shims; collection URLs and unknown ids → 404; glossary adds Squad. Resolution: [#9](https://github.com/karldoyledev/BlackoutRugbyAnalysis/issues/9).
